...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        if len(state.stack)==0:
            new_word=[None,None,None]
        elif len(state.stack)==1:
            new_word=[state.stack[-1],None,None]
        elif len(state.stack)==2:
            new_word=[state.stack[-1],state.stack[-2],None]
        else:
            new_word=[state.stack[-1],state.stack[-2],state.stack[-3]]
        if len(state.buffer)==0:
            new_word+=[None,None,None]
        elif len(state.buffer)==1:
            new_word += [state.buffer[-1],None,None]
        elif len(state.buffer)==2:
            new_word+=[state.buffer[-1],state.buffer[-2],None]
        else:
            new_word += [state.buffer[-1], state.buffer[-2], state.buffer[-3]]
        for word in new_word:
            if not word:
                ...
                input.append(1)
            elif words[word].lower() not in self.word_vocab.keys():
                input.append(2)
            else:
                input.append(self.word_vocab[words[word].lower()])
